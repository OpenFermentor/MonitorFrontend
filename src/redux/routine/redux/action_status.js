...
const actionStatusReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case START_ROUTINE_SUCCESS:
      return setRunningRoutine(state, action);
    case STOP_ROUTINE_SUCCESS:
      return clearRunningRoutine(state, action);
    case SET_DATA_RANGE:
      return setDataRange(state, action);

    case SET_SELECTED_ROUTINE:
      return setSelectedRoutine(state, action);
    case FETCH_SUCCESS:
      return setSelectedRoutine(state, action);
    case CLEAR_SELECTED_ROUTINE:
      return clearSelectedRoutine(state, action);

    case FETCH_ROUTINES_REQUEST:
      return clearSelectedRoutine(state, action);
    case FETCH_ROUTINES_SUCCESS:
      return addRoutinesPagination(state, action);
...
const clearRunningRoutine = state => merge(state, { runningRoutine: null });

const setDataRange = (state, { start, end }) =>
  merge(state, { dataRangeStart: start, dataRangeEnd: end });

const clearSelectedRoutine = state => merge(state, { selectedRoutine: null });

const setSelectedRoutine = (state, { routine }) =>
  merge(state, { selectedRoutine: routine.id, searchResults: null });

const clearSearchResults = state => merge(state, { searchResults: null });

const setSearchTerm = (state, { searchTerm }) => merge(state, { searchTerm });
